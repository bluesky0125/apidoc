...
public struct Secp256k1 {
    ...
    public static func keyPair(privateBytes: [UInt8]) throws -> (PrivateKey, PublicKey) {
        let privkey = privateBytes
        //let status = SecRandomCopyBytes(kSecRandomDefault, privkey.count, &privkey)
        guard let pubkey = try? derivePublicKey(for: privkey) else {
