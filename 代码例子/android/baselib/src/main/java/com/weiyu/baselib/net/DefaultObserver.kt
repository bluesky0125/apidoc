...
//    }
    ...
    open fun onFail(response: T) {
        if (response.code == 10028) {
            EventBus.getDefault().post(UserUtil.instance.getUser())
        }
    ...
    private fun onException(reason: ExceptionReason) {
        when (reason) {
            DefaultObserver.ExceptionReason.CONNECT_ERROR -> activity.toast("error")
            DefaultObserver.ExceptionReason.CONNECT_TIMEOUT -> activity.toast("time out")
            DefaultObserver.ExceptionReason.BAD_NETWORK -> activity.toast("bad net")
